...
public class ReadIndex {
     ...
     */
    record Pending(NodeId from, long index, long seq) {};
    ...
    public List<Pending> drainAcked(NodeId leader, MembershipConfig mc) {
        ...
        var majorityAckedSeq = mc.majorityAgreed(voter ->
                voter.equals(leader)
                        ? OptionalLong.of(Long.MAX_VALUE)
                        : OptionalLong.of(peerAckedSeq.getOrDefault(voter, 0L))
        );

        var ackedReads = new ArrayList<Pending>();
        while (!pendingReads.isEmpty() && pendingReads.peek().seq() <= majorityAckedSeq) {
