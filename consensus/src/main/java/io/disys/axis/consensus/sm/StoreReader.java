...
public final class StoreReader {
    ...
    public StoreReader(NodeContext context) {
        this.context = context;
    }

    public GetResponse get(Reader reader, GetRequest req) {
        var result = reader.get(req.getKey().toByteArray());
        var response = GetResponse.newBuilder()
                .setHeader(context.header(reader.revision()));
        result.ifPresent(r -> response.setKv(KeyValCodec.toKeyVal(r)));
        return response.build();
    }

    public GetAtResponse getAt(Reader reader, GetAtRequest req) {
        var result = reader.getAt(req.getKey().toByteArray(), req.getRevision());
        var header = context.header(reader.revision());
        return switch (result) {
            case SnapshotResult.Ok<java.util.Optional<Record>> ok -> {
                ...
                yield GetAtResponse.newBuilder().setHeader(header).setOk(ok_).build();
            }
            case SnapshotResult.Compacted<java.util.Optional<Record>> c ->
                GetAtResponse.newBuilder()
                        .setHeader(header)
                        ...
                        .build();
            case SnapshotResult.Future<java.util.Optional<Record>> f ->
                GetAtResponse.newBuilder()
                        .setHeader(header)
    ...
    public RangeResponse range(Reader reader, RangeRequest req) {
    ...
    public RangeAtResponse rangeAt(Reader reader, RangeAtRequest req) {
        ...
        var header = context.header(reader.revision());
        return switch (result) {
            case SnapshotResult.Ok<Page<Record>> ok ->
                RangeAtResponse.newBuilder()
                        .setHeader(header)
    ...
    public KeysResponse keys(Reader reader, KeysRequest req) {
        var page = req.hasOptions()
    ...
    public KeysAtResponse keysAt(Reader reader, KeysAtRequest req) {
        var result = req.hasOptions()
        ...
        var header = context.header(reader.revision());
        return switch (result) {
            case SnapshotResult.Ok<Page<byte[]>> ok ->
                KeysAtResponse.newBuilder()
                        .setHeader(header)
                        .setOk(KeysResult.newBuilder()
                                .addAllKeys(ok.value().items().stream().map(ByteString::copyFrom).toList())
                                .setMore(ok.value().more()))
                        .build();
            case SnapshotResult.Compacted<Page<byte[]>> c ->
                KeysAtResponse.newBuilder()
                        .setHeader(header)
                        .setCompacted(Compacted.newBuilder()
                                .setFirstVisibleRevision(c.firstVisibleCommitSeq())
                                .setRequestedRevision(c.requestedCommitSeq()))
                        .build();
            case SnapshotResult.Future<Page<byte[]>> f ->
                KeysAtResponse.newBuilder()
                        .setHeader(header)
                        .setFuture(Future.newBuilder()
                                .setLastVisibleRevision(f.lastVisibleCommitSeq())
    ...
    public CountResponse count(Reader reader, CountRequest req) {
    ...
    public CountAtResponse countAt(Reader reader, CountAtRequest req) {
        var result = req.hasOptions()
        ...
        var header = context.header(reader.revision());
        return switch (result) {
            case SnapshotResult.Ok<Long> ok ->
                CountAtResponse.newBuilder()
                        .setHeader(header)
                        .setOk(CountResult.newBuilder().setCount(ok.value()))
                        .build();
            case SnapshotResult.Compacted<Long> c ->
                CountAtResponse.newBuilder()
                        .setHeader(header)
                        .setCompacted(Compacted.newBuilder()
                                .setFirstVisibleRevision(c.firstVisibleCommitSeq())
                                .setRequestedRevision(c.requestedCommitSeq()))
                        .build();
            case SnapshotResult.Future<Long> f ->
                CountAtResponse.newBuilder()
                        .setHeader(header)
                        .setFuture(Future.newBuilder()
                                .setLastVisibleRevision(f.lastVisibleCommitSeq())
